var resUtil = {
  ...
  // 所有城池为其设置太守
  initCitySatrap: function(period, cityId) {
    var heros = resUtil.getCityHeros(period, cityId);
    resUtil.getHerosSortByIndex(heros, 4, 'desc');
  },
  ...
  setCitySatrap: function(period, cityId) {
    ...
    if (lordId = 99) {
      // 设置城池太守值为 0
      return setACityInfo(period, cityId, 2, 0);
    }
    ...
    if (isExist) {
      return setACityInfo(period, cityId, 2, parseInt(heroId) + 1);
    }
    // 获取城池中以智能降序排行的列表
    var newHeros = resUtil.getHerosSortByIndex(heros, 4, 'desc');
    // 取出 第 1 个序号，设其为太守
    setACityInfo(period, cityId, 2, parseInt(newHeros.split(' ')[0]) + 1);
  },
